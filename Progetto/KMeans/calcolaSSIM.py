...
def cssim(image1,image2,i,j):
    s=ssim(img1,img2, multichannel=True)#calcola il ssim dell'immagine di partenza con quella ricostruita
    key=str(i)
    key2=str(j)
    ssimdati[str(key),str(key2)]=s #aggiungo i ssim per poi salvarli
